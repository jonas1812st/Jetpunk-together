...
function setReadyState(session_id, state) {
  // state is either 0 (unready) or 1 (ready)
  const info = db.run("UPDATE users SET ready = ? WHERE session_id = ?", [
    state,
    session_id,
  ]);
...
function setUserScore(session_id, score, possible) {
  const info = db.run("UPDATE users SET score = ? WHERE session_id = ?;", [
    score + "/" + possible,
    session_id,
  ]);
